...
# Desired columns for full export – only existing ones will be written
DESIRED_FULL_COLS = [
...
# -------------------------------------------------
# ALL VALIDATION FUNCTIONS (SAFE & WORKING)
# -------------------------------------------------
...
def to_excel_full_data(data_df, report_df):
    merged = data_df.merge(
        report_df[["ProductSetSid","Status","Reason","Comment","FLAG","SellerName"]],
        left_on="PRODUCT_SET_SID", right_on="ProductSetSid", how="left"
    )
    available_cols = [c for c in DESIRED_FULL_COLS if c in merged.columns]
    out = BytesIO()
    with pd.ExcelWriter(out, engine='xlsxwriter') as writer:
        merged[available_cols].to_excel(writer, index=False, sheet_name="ProductSets")
    out.seek(0)
    return out.getvalue()
...
if uploaded:
    try:
        ...
        # Sidebar Downloads
        st.sidebar.markdown("---")
        st.sidebar.download_button("Final Report", to_excel(display_report, support_files['reasons']), f"Final_Report_{country}.xlsx")
        st.sidebar.download_button("Full Data Export", to_excel_full_data(df, report_df), f"Full_Data_{country}.xlsx")
        ...
        with col1:
            st.download_button("FINAL REPORT – ALL SELLERS", to_excel(report_df, support_files['reasons']), "FINAL_ALL.xlsx")
        with col2:
            st.download_button("FULL DATA – ALL SELLERS", to_excel_full_data(df, report_df), "FULL_DATA_ALL.xlsx")
