...
if uploaded_file is not None:
    try:
        ...
        # Check the columns in the uploaded data
        st.write("CSV file loaded successfully. Column names in the file:")
        st.write(data.columns.tolist())

        # Check for required columns
        required_columns = ['PRODUCT_SET_ID', 'PRODUCT_SET_SID', 'NAME', 'BRAND', 'CATEGORY', 'PARENTSKU', 'SELLER_NAME', 'GLOBAL_PRICE', 'GLOBAL_SALE_PRICE', 'COLOR', 'CATEGORY_CODE', 'VARIATION']
        ...
        if missing_columns:
            ...
            if not data.empty:
                ...
                if not brand_in_name.empty:
                    ...
                    total_flagged_products += flagged_count
                    st.error(f"Found {flagged_count} products with the BRAND name in NAME.")
