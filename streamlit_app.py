...
if uploaded_files:
    ...
    if 'last_sig' not in st.session_state or st.session_state.last_sig != current_sig:
        with st.spinner("Processing files..."):
            ...
            # Initialize Report (All approved initially for manual QC)
            report_rows = [{
...
if not st.session_state.final_report.empty:
            ...
            with st.expander(f"📸 Review Approved Images ({len(valid_items)} total)", expanded=True):
                ...
                for i, (_, row) in enumerate(curr_page_data.iterrows()):
                    with grid[i % 3]:
                        ...
                        is_valid_str = isinstance(img_val, str) and len(str(img_val)) > 5
                        
                        if is_valid_str and str(img_val).lower() != 'nan':
                            clean_url = img_val.strip()
                            if clean_url.startswith('//'): clean_url = 'https:' + clean_url
                            
                            try:
                                st.image(clean_url, use_container_width=True)
                                with st.popover("🔍 Zoom"):
                                    st.image(clean_url, caption=f"ID: {sid}")
                            except:
                                st.error("🚫 Loading error")
                                st.caption(f"[View Link]({clean_url})")
                        else:
                            st.warning("EMPTY")
                            st.info("Missing URL")
                        ...
                        c1, c2 = st.columns(2)
                        with c1: poor = st.checkbox("Poor Quality", key=f"p_{sid}_{pg}")
                        with c2: wrong = st.checkbox("Wrong Cat", key=f"w_{sid}_{pg}")
                ...
                if st.session_state.image_review_decisions:
                    st.divider()
                    st.warning(f"Pending: {len(st.session_state.image_review_decisions)} rejections.")
    ...
    with col_ex1:
        ...
        st.download_button("Download Full QC Report (.xlsx)", output.getvalue(), "QC_Report.xlsx")

    with col_ex2:
        rej_only = st.session_state.final_report[st.session_state.final_report['Status'] == 'Rejected']
        if not rej_only.empty:
            st.download_button("Download Rejections Only (.csv)", rej_only.to_csv(index=False), "rejections.csv", "text/csv")
