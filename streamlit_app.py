...
COLUMN_MAPPING = {
    ...
    'color': 'COLOR',
    'Color': 'COLOR',
    'categories': 'CATEGORY_PATH_RAW',
    'Category': 'CATEGORY_PATH_RAW',
...
def check_suspected_fake_price(row, fake_config_df):
    ...
    if fake_config_df is None or fake_config_df.empty:
        return None
    
    brand = str(row.get('BRAND', '')).strip().lower()
    cat_code = str(row.get('CATEGORY_CODE', ''))
    
    if not cat_code or cat_code == 'N/A':
        return None
...
def validate_jerseys(row, jerseys_df):
    ...
    3. Regex Match Name
    """
    if jerseys_df is None or jerseys_df.empty:
        return None
...
def validate_restricted_brands(row, restricted_brands_df):
    if restricted_brands_df is None:
        return None
...
def validate_generic_brand_name(row, brands_list):
    """
    Checks if a Generic-branded product has a real brand name at the start.
    Handles multi-word brands (e.g., 'Dr Rashel').
    """
    if not brands_list:
        return None
    
    brand = str(row.get('BRAND', '')).strip().lower()
    if brand not in ['generic', 'fashion', 'no brand', 'other', '', 'nan']:
        return None
    ...
    for real_brand in sorted_brands:
        # Check if the name starts with this brand
        # Use word boundary at the end to ensure complete word match
        pattern = r'^' + re.escape(real_brand) + r'\b'
        if re.search(pattern, name, re.IGNORECASE):
            return f"Generic Brand Mismatch: Name starts with '{real_brand}' but brand is Generic"
...
with st.sidebar:
    ...
    with st.spinner("Loading System Rules..."):
        ...
        if suspected_fake_df is None:
            suspected_fake_df = load_config_file("suspected_fake.csv", "csv")
        ...
        # NEW: Load brands list for Generic validation
        brands_list = load_config_file("brands.txt", "txt")
...
if prod_file and path_to_code:
    ...
    for idx, row in df.iterrows():
        ...
        # NEW: Generic Brand Name Check
        res = validate_generic_brand_name(row, brands_list)
        if res:
            reasons.append(res)
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        export_cols = ['PRODUCT_SET_SID', 'NAME', 'BRAND', 'CATEGORY_CODE', 'SELLER_NAME', 
                       'GLOBAL_SALE_PRICE', 'Validation_Status', 'Validation_Reason']
        if 'categories' in raw_df.columns:
        ...
        ws.conditional_format(1, idx, len(final_df), idx, 
                            {'type': 'cell', 'criteria': 'equal', 'value': '"Rejected"', 'format': red})
        ws.conditional_format(1, idx, len(final_df), idx, 
                            {'type': 'cell', 'criteria': 'equal', 'value': '"Approved"', 'format': green})
