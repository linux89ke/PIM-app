    ...
    try:
        ...
        if not data.empty:
            ...
            def check_perfume(row):
                ...
                return False
            flagged_perfumes = data[data.apply(check_perfume, axis=1)]
            ...
            # Display sections for each flag result
            ...
            with st.expander(f"Perfume price issue ({flagged_perfumes_count} products)"):
                st.write(flagged_perfumes[['PRODUCT_SET_ID', 'PRODUCT_SET_SID', 'NAME', 'BRAND', 'CATEGORY', 'PARENTSKU', 'SELLER_NAME', 'GLOBAL_PRICE']] if flagged_perfumes_count > 0 else "No products flagged.")
            ...
            with st.expander(f"BRAND name repeated in NAME ({brand_in_name_count} products)"):
                st.write(brand_in_name[['PRODUCT_SET_ID', 'PRODUCT_SET_SID', 'NAME', 'BRAND', 'CATEGORY', 'PARENTSKU', 'SELLER_NAME']] if brand_in_name_count > 0 else "No products flagged.")
