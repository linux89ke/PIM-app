...
def load_all_support_files() -> Dict:
    ...
    files = {
        ...
        # FIX: Correctly loading the suspected_fake file using the appropriate helper
        # Assuming original Excel file name is 'suspected_fake.xlsx'
...
def check_suspected_fake_products(data: pd.DataFrame, suspected_fake_df: pd.DataFrame, fx_rate: float = 132.0) -> pd.DataFrame:
    ...
    try:
        ...
        if ref_data.columns.str.contains('Price').any(): # Check if 'Price' is in the column names of the DataFrame loaded by load_excel_file
