...
def load_txt_file(filename: str) -> List[str]:
    ...
    NOTE: Removed st.warning for missing files to prevent UI clutter.
    The downstream validation check handles the empty list gracefully.
    """
    try:
        ...
        # Log a warning internally, but don't show a Streamlit warning.
        logger.warning(f"{filename} not found, related check disabled.")
        return []
...
def load_flags_mapping() -> Dict[str, Tuple[str, str]]:
    """
    Load flags.xlsx for reason/comment mapping, including the new 'Jersey flag' and 'Restricted Brand'.
    """
    try:
        # Manual mapping based on your actual flags.xlsx content + new flags
        flag_mapping = {
            'Sensitive words': (
            ...
            # --- NEW JERSEY FLAG ADDED HERE ---
            'Jersey flag': (
                '1000007 - Other Reason',
                "Product listing includes the term 'Jersey'. Please ensure the item is not a counterfeit or restricted team apparel. Contact Seller Support for approval."
            ),
            # --- NEW RESTRICTED BRAND FLAG ADDED HERE ---
            'Restricted Brand': (
                '1000003 - Restricted Brand',
                "This is a Restricted brand. To sell this brand, you need to provide proof of originality or purchase invoices that the products are original.\nPlease send the required documents to Seller Support or your Sales Account Manager to get approval for listing"
            ),
            # --------------------------------------------
...
def load_all_support_files() -> Dict:
    ...
    files = {
        ...
        'restricted_brands': load_txt_file('restricted_brands.txt'), # Handled silently by load_txt_file
...
def check_jersey_flag(data: pd.DataFrame) -> pd.DataFrame:
    """Check for the word 'jersey' in product names."""
    ...
    # Simple check: flags anything with the word 'jersey'
    mask = data['NAME_LOWER'].str.contains(r'\bjersey\b', na=False)

    return data[mask].drop(columns=['NAME_LOWER'])

def check_restricted_brand(data: pd.DataFrame, restricted_brands: List[str]) -> pd.DataFrame:
    """Check if the BRAND is in the restricted list."""
    if 'BRAND' not in data.columns or not restricted_brands:
        return pd.DataFrame(columns=data.columns)

    data = data.copy()
    data['BRAND_LOWER'] = data['BRAND'].astype(str).str.strip().str.lower()
    
    mask = data['BRAND_LOWER'].isin([b.lower() for b in restricted_brands])

    return data[mask].drop(columns=['BRAND_LOWER'])
...
def validate_products(
    ...
    validations = [
        ...
        # --- NEW FLAGS INTEGRATED HERE (High Priority) ---
        ("Jersey flag", check_jersey_flag, {}),
        ("Restricted Brand", check_restricted_brand, {'restricted_brands': support_files['restricted_brands']}),
        # -------------------------------------------------
