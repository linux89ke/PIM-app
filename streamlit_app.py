...
def load_flags_mapping() -> Dict[str, Tuple[str, str]]:
    """Load flags.xlsx with hardcoded mapping (fallback if file missing)"""
    try:
        ...
        for _, row in flags_df.iterrows():
            flag = str(row.get('rejection_reason_code', '')).strip()
            comment = str(row.get('Comment', '')).strip()
            if flag:
                flag_mapping[flag] = (flag, comment)
        logger.info(f"Loaded {len(flag_mapping)} flag mappings from flags.xlsx")
        return flag_mapping
    except FileNotFoundError:
        logger.warning("flags.xlsx not found – using fallback mapping")
        st.warning("flags.xlsx not found – using fallback reason mapping")
        return {
            'Sensitive words': ('1000001 - Brand NOT Allowed', 'Your listing includes restricted brands.'),
            'BRAND name repeated in NAME': ('1000002 - Kindly Ensure Brand Name Is Not Repeated In Product Name', 'Do not repeat brand in product name.'),
            'Missing COLOR': ('1000005 - Kindly confirm the actual product colour', 'Color must be in title or color field.'),
            'Prohibited products': ('1000007 - Other Reason', 'Product is prohibited.'),
            'Duplicate product': ('1000007 - Other Reason', 'Duplicate listing.'),
            'Single-word NAME': ('1000008 - Kindly Improve Product Name Description', 'Use full descriptive title.'),
            'Generic BRAND Issues': ('1000014 - Kindly request for the creation of this product\'s actual brand name', 'Use correct brand or request creation.'),
            'Counterfeit Sneakers': ('1000023 - Confirmation of counterfeit product', 'Counterfeit product detected.'),
            'Seller Approve to sell books': ('1000028 - Kindly Contact Jumia Seller Support', 'Seller not approved for books.'),
            'Seller Approved to Sell Perfume': ('1000028 - Kindly Contact Jumia Seller Support', 'Seller not approved for perfume.'),
            'Perfume Price Check': ('1000029 - Kindly Contact Jumia Seller Support To Verify', 'Price too low – authenticity check needed.'),
        }
    except Exception as e:
        logger.error(f"Error loading flags.xlsx: {e}")
        return {}
...
def check_seller_approved_for_perfume(data: pd.DataFrame, perfume_category_codes: List[str], approved_perfume_sellers: List[str], sensitive_perfume_brands: List[str]) -> pd.DataFrame:
    ...
    fake_brands = ['designers collection', 'smart collection', 'generic', 'original', 'designer', 'fashion']  # Fixed comma
...
def validate_products(data: pd.DataFrame, support_files: Dict, country_validator: CountryValidator) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    ...
    for i, (flag_name, check_func, func_kwargs) in enumerate(validations):
        ...
        try:
            ...
            validation_results_dfs[flag_name] = result_df if 'PRODUCT_SET_SID' in result_df.columns else pd.DataFrame(columns=data.columns)
    ...
    for flag_name, _, _ in validations:
        ...
        if validation_df.empty or 'PRODUCT_SET_SID' not in validation_df.columns:
            continue
        rejection_reason, comment = flags_mapping.get(flag_name, ("1000007 - Other Reason", f"Flagged: {flag_name}"))
        flagged_sids_df = pd.merge(validation_df[['PRODUCT_SET_SID']], data, on='PRODUCT_SET_SID', how='left')
...
def to_excel_full_data(data_df: pd.DataFrame, final_report_df: pd.DataFrame) -> BytesIO:
    try:
        ...
        merged_df = pd.merge(data_df_copy, final_report_df_copy[["ProductSetSid", "Status", "Reason", "Comment", "FLAG", "SellerName"]], left_on="PRODUCT_SET_SID", right_on="ProductSetSid", how='left')
        if merged_df.empty:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            try:
                if 'CATEGORY' in merged_df.columns and not merged_df['CATEGORY'].isna().all():
                    ...
                    category_rejections.insert(0, 'Rank', arrange(1, len(category_rejections) + 1))
                    sellers_data_rows.append(pd.DataFrame([['', '', '', '']]))
...
def to_excel(report_df: pd.DataFrame, reasons_config_df: pd.DataFrame, sheet1_name: str = "ProductSets", sheet2_name: str = "RejectionReasons") -> BytesIO:
    output = BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
...
# Audit Logging
# -------------------------------------------------
def log_validation_run(country: str, file_name: str, total_rows: int, approved: int, rejected: int, user: str = None) -> None:
    audit_entry = {
...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        try:
            dtype_spec = {'CATEGORY_CODE': str, 'PRODUCT_SET_SID': str, 'PARENTSKU': str, 'ACTIVE_STATUS_COUNTRY': str}
            ...
            log_validation_run(country=country, file_name=uploaded_file.name, total_rows=len(data), approved=len(approved_df), rejected=len(rejected_df))
            ...
            if 'SELLER_NAME' in data.columns and 'report_with_seller' in locals():
                sellers_to_display = selected_sellers if 'All Sellers' not in selected_sellers else seller_options[1:]
...
# ================================
with tab2:
    st.header("Weekly Validation Analysis")
    st.info("Upload multiple daily validation reports (Excel) to analyze trends over the week.")
    uploaded_files = st.file_uploader("Upload Daily Reports (Excel)", type=['xlsx'], accept_multiple_files=True, key="weekly")
    if uploaded_files:
        all_reports = []
        ...
        if all_reports:
            ...
            st.success(f"Combined {len(all_reports)} reports with {len(combined)} total rows")
            col1, col2 = st.columns(2)
            with col1:
                st.subheader("Rejection Rate Trend")
                daily_stats = combined.groupby('Source_File').apply(lambda x: pd.Series({
                    'Total': len(x),
                    'Rejected': (x['Status'] == 'Rejected').sum(),
                    'Rate': round((x['Status'] == 'Rejected').sum() / len(x) * 100, 1) if len(x) > 0 else 0
                })).reset_index()
                st.dataframe(daily_stats)
            with col2:
                st.subheader("Top Rejection Reasons")
                reason_counts = combined[combined['Status'] == 'Rejected']['Reason'].value_counts().head(10)
                st.bar_chart(reason_counts)
            st.download_button("Download Combined Report", to_excel(combined, pd.DataFrame()), "Weekly_Combined_Report.xlsx")
...
with tab3:
    ...
    try:
        ...
        if 'rejection_rate' in df.columns:
            st.line_chart(df.set_index('timestamp')['rejection_rate'])
    except Exception as e:
        st.info("No audit data yet. Run validations to generate logs.")
