...
import traceback
import json  # Added for JSONL parsing in Data Lake
# -------------------------------------------------
...
def check_suspected_fake_products(data: pd.DataFrame, suspected_fake_df: pd.DataFrame) -> pd.DataFrame:
    """
    Check for suspected fake products based on brand, category, and price thresholds
    
    Args:
        data: Product data to validate
        suspected_fake_df: Reference data with Brand, Price, and category codes
    ...
    try:
        # Parse the suspected_fake.xlsx structure
        # First row has brands, second row has prices, remaining rows have category codes
        brands = suspected_fake_df.iloc[0].dropna().tolist()  # First row = brands
        prices = suspected_fake_df.iloc[1].dropna().tolist()  # Second row = price thresholds
        
        # Build mapping of brand -> (price_threshold, category_codes)
        brand_config = {}
        
        for col_idx, brand in enumerate(brands):
            if pd.isna(brand) or brand == 'Brand':
                continue
            
            # Get price threshold for this brand
            try:
                price_threshold = float(prices[col_idx]) if col_idx < len(prices) else 0
            except (ValueError, TypeError):
                price_threshold = 0
            
            # Get all category codes for this brand (from row 3 onwards)
            category_codes = []
            for row_idx in range(2, len(suspected_fake_df)):  # Start from row 3 (index 2)
                cat_code = suspected_fake_df.iloc[row_idx, col_idx]
                if pd.notna(cat_code) and str(cat_code).strip():
                    category_codes.append(str(cat_code).strip())
        ...
        if not brand_config:
            logger.warning("No valid brand configurations found in suspected_fake.xlsx")
            return pd.DataFrame(columns=data.columns)
        ...
        # Calculate price to use (sale price if available, else regular price)
        check_data['price_to_check'] = check_data['GLOBAL_SALE_PRICE'].where(
        ...
        if 'CURRENCY' in check_data.columns:
            check_data['price_usd'] = check_data.apply(
                lambda row: row['price_to_check'] / FX_RATE 
                if str(row.get('CURRENCY', '')).upper() == 'KES' 
                else row['price_to_check'],
                axis=1
            )
        else:
            # Assume KES if currency column not present
            check_data['price_usd'] = check_data['price_to_check'] / FX_RATE
        
        # Flag products matching criteria
        flagged_mask = pd.Series([False] * len(check_data), index=check_data.index)
        
        for brand_lower, config in brand_config.items():
            # Match brand
            brand_match = check_data['BRAND_LOWER'] == brand_lower
            
            # Match category codes
            category_match = check_data['CATEGORY_CODE_STR'].isin(config['category_codes'])
            
            # Match price (below threshold)
            price_match = check_data['price_usd'] < config['price_threshold']
            
            # Combine conditions
            flagged_mask |= (brand_match & category_match & price_match)
        
        # Return flagged products
        flagged_products = check_data[flagged_mask].copy()
        
        # Drop temporary columns
        cols_to_drop = ['BRAND_LOWER', 'CATEGORY_CODE_STR', 'price_to_check', 'price_usd']
        flagged_products = flagged_products.drop(columns=[c for c in cols_to_drop if c in flagged_products.columns])
        
        logger.info(f"Suspected fake products check: {len(flagged_products)} products flagged")
        
        return flagged_products
