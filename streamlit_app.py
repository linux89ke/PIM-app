...
def load_all_support_files() -> Dict:
    ...
    files = {
        ...
        'suspected_fake': load_excel_file('suspected_fake.xlsx'),  # NEW FILE
    }
...
# NEW VALIDATION: Suspected Fake Products
def check_suspected_fake_products(data: pd.DataFrame, suspected_fake_df: pd.DataFrame) -> pd.DataFrame:
    if not {'CATEGORY_CODE', 'BRAND', 'GLOBAL_SALE_PRICE', 'GLOBAL_PRICE'}.issubset(data.columns):
        return pd.DataFrame(columns=data.columns)
    ...
    try:
        ...
        for col_idx, brand in enumerate(brands):
            if pd.isna(brand) or str(brand).strip() == '' or str(brand).strip().lower() == 'brand':
                continue
            try:
            ...
            category_codes = [
                str(suspected_fake_df.iloc[row_idx, col_idx]).strip()
                for row_idx in range(2, len(suspected_fake_df))
                if pd.notna(suspected_fake_df.iloc[row_idx, col_idx])
            ]

            if category_codes:
                brand_config[str(brand).strip().lower()] = {
                    'price_threshold': price_threshold,
                    'category_codes': category_codes
                }

        if not brand_config:
            logger.warning("No valid brand configurations in suspected_fake.xlsx")
            return pd.DataFrame(columns=data.columns)

        check = data.copy()
        check['BRAND_LOWER'] = check['BRAND'].astype(str).str.strip().str.lower()
        check['CATEGORY_CODE_STR'] = check['CATEGORY_CODE'].astype(str).str.strip()
        check['price_to_check'] = check['GLOBAL_SALE_PRICE'].where(
            (check['GLOBAL_SALE_PRICE'].notna()) & (check['GLOBAL_SALE_PRICE'] > 0),
            check['GLOBAL_PRICE']
        )
        if 'CURRENCY' in check.columns:
            check['price_usd'] = check.apply(
                lambda r: r['price_to_check'] / FX_RATE if str(r.get('CURRENCY', '')).upper() == 'KES' else r['price_to_check'],
                axis=1
            )
        else:
            check['price_usd'] = check['price_to_check'] / FX_RATE

        flagged = pd.Series([False] * len(check), index=check.index)
        for brand_lower, cfg in brand_config.items():
            brand_match = check['BRAND_LOWER'] == brand_lower
            cat_match = check['CATEGORY_CODE_STR'].isin(cfg['category_codes'])
            price_match = check['price_usd'] < cfg['price_threshold']
            flagged |= (brand_match & cat_match & price_match)

        result = check[flagged].copy()
        result.drop(columns=['BRAND_LOWER', 'CATEGORY_CODE_STR', 'price_to_check', 'price_usd'], inplace=True, errors='ignore')
        logger.info(f"Suspected Fake Products check → {len(result)} flagged")
        return result

    except Exception as e:
        logger.error(f"Error in check_suspected_fake_products: {e}", exc_info=True)
        return pd.DataFrame(columns=data.columns)
...
def validate_products(data: pd.DataFrame, support_files: Dict, country_validator: CountryValidator) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    ...
    validations = [
        ...
        ("Suspected Fake Products", check_suspected_fake_products,
    ...
    for flag_name, _, _ in validations:
        ...
        rejection_reason, comment = flags_mapping.get(flag_name, ("1000007 - Other Reason", f"Product flagged by validation: {flag_name}"))
...
def to_excel_full_data(data_df: pd.DataFrame, final_report_df: pd.DataFrame) -> BytesIO:
    try:
        ...
        merged_df = pd.merge(data_df_copy, final_report_df_copy[["ProductSetSid", "Status", "Reason", "Comment", "FLAG", "SellerName"]],
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            # (rest of the function unchanged - omitted for brevity)
        output.seek(0)
        return output
    except Exception as e:
...
if not support_files['flags_mapping']:
    st.error("Critical: flags.xlsx could not be loaded. Please ensure the file exists.")
    st.stop()
...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            log_validation_run(country=country, file_name=uploaded_file.name, total_rows=len(data),
                               approved=len(approved_df), rejected=len(rejected_df))

            # (rest of the UI code unchanged - omitted for brevity, but fully functional)
            ...
            for title, df_flagged in individual_flag_dfs.items():
                with st.expander(f"**{title}** ({len(df_flagged)} products)", expanded=False):
                    if not df_flagged.empty:
                        ...
                        st.download_button(f"Export {title}", to_excel_flag_data(df_flagged.copy(), title),
                                           f"{file_prefix}_{safe}_{current_date}.xlsx", key=f"flag_{safe}")
                    else:
...
# (Tabs 2 and 3 remain unchanged - included in full version if needed)

st.success("Product Validation Tool v2.1 – Now with Suspected Fake Products detection!")
