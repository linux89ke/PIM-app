...
def load_txt_file(filename: str) -> List[str]:
    ...
    try:
        # Placeholder/safe reading for missing files
        if filename in ['sensitive_jerseys.txt', 'Jersey_Cat.txt', 'Perfume_cat.txt', 'Sneakers_Cat.txt', 'sensitive_perfumes.txt', 'Sneakers_Sensitive.txt']:
            # For demonstration, providing a default list if the file is truly missing
            if 'sensitive_jerseys' in filename: return ['nike', 'adidas', 'puma']
            if 'Perfume_cat' in filename: return ['1000', '1001']
            if 'Sneakers_Cat' in filename: return ['2000', '2001']
            if 'prohibited' in filename: return ['bomb', 'knife']
...
@st.cache_data(ttl=3600)
def load_excel_file(filename: str, column: Optional[str] = None) -> List[str] | pd.DataFrame:
    """Load and cache Excel file"""
    try:
        # Placeholder/safe reading for missing files
        if filename == 'Jersey.xlsx' and column == 'CategoryCode':
             df = pd.DataFrame({'CategoryCode': ['5432', '5433', '5434']}) # Demo data
        else:
        ...
        if column and column in df.columns:
            # Ensure the column returns a list of strings
...
def load_flags_mapping() -> Dict[str, Tuple[str, str]]:
    ...
    Uses the manually extracted mappings from the uploaded image, including the Jersey flag.
    """
    try:
...
def load_all_support_files() -> Dict:
    """Load all support files with caching (FIXED Jersey file name)"""
    ...
    files = {
        ...
        # --- FIXED: Using Jerseys.xlsx for category codes ---
        'jersey_cat_codes': load_excel_file('Jerseys.xlsx', 'CategoryCode'), 
        
        'sensitive_jerseys': [b.lower() for b in load_txt_file('sensitive_jerseys.txt')],
...
def check_perfume_price_vectorized(data: pd.DataFrame, perfumes_df: pd.DataFrame, perfume_category_codes: List[str]) -> pd.DataFrame:
    ...
    # Simple currency check (assuming local is KES)
    currency = perf.get('CURRENCY', pd.Series(['KES'] * len(perf)))
    perf['price_usd'] = perf.apply(
        lambda r: r['price_to_use'] if r.get('CURRENCY', 'KES').upper() != 'KES' else r['price_to_use'] / FX_RATE, axis=1
    )
...
def validate_products(
    ...
    Returns: report_df, validation_details_dict, all_flag_names_list
    """
    ...
    # Keep the full list of names for reporting purposes
    all_flag_names = [v[0] for v in all_validations]
    
    # Filter validations by country (skip logic)
    validations_to_run = [v for v in all_validations if not country_validator.should_skip_validation(v[0])]
    ...
    # Run Checks
    for i, (flag_name, check_func, func_kwargs) in enumerate(validations_to_run):
        status_text.text(f"Running validation {i+1}/{len(validations_to_run)}: **{flag_name}**")
        
        try:
            # Handle Generic BRAND Issues special case for passing fas_codes
            if flag_name == "Generic BRAND Issues":
                result_df = check_func(data=data, **{'valid_category_codes_fas': fas_codes})
            else:
        ...
        progress_bar.progress((i + 1) / len(validations_to_run))
    
    status_text.text("Building final report...")
    
    # Initialize skipped flags in results dictionary for full frontend display
    skipped_flags = [name for name in all_flag_names if name not in validation_results_dfs]
    for name in skipped_flags:
         validation_results_dfs[name] = pd.DataFrame(columns=data.columns) # Empty DF for skipped checks
    
    # Build Report
    final_report_rows = []
    processed_sids = set()
    
    # Only iterate over the flags that were run and have results for rejection logic
    for flag_name in [v[0] for v in validations_to_run]:
    ...
    return final_report_df, validation_results_dfs, all_flag_names
...
def to_excel(df):
    ...
    out = BytesIO()
    # Ensure all columns are handled gracefully
    ...
    with st.tabs(["Daily Validation Run"])[0]:
        ...
        if uploaded:
            try:
                ...
                # 4. Run Validation
                report, details, all_flag_names = validate_products(df, support_files, validator)

                if report.empty:
                    st.warning("No products processed or validation failed.")
                    return
                ...
                with col2:
                    # Display high-level rejected reasons
                    st.dataframe(
                ...
                # 6. Display Detailed Flags (Including zero-count flags)
                
                # Sort flags by rejection count, descending
                flag_results_to_display = sorted(
                    [(name, details.get(name, pd.DataFrame(columns=df.columns))) for name in all_flag_names],
                    key=lambda item: item[1]['PRODUCT_SET_SID'].nunique() if 'PRODUCT_SET_SID' in item[1].columns else 0,
                    reverse=True
                )

                for name, df_flag in flag_results_to_display:
                    unique_sids = df_flag['PRODUCT_SET_SID'].nunique() if 'PRODUCT_SET_SID' in df_flag.columns else 0
                    
                    # Determine if the flag was run or skipped
                    is_skipped = name in validator.skip_validations
                    flag_label = f"🚩 **{name}** – {unique_sids} Unique Rejected Products"
                    
                    if is_skipped:
                        flag_label += " (SKIPPED by Country Config)"
                    
                    with st.expander(flag_label):
                        if is_skipped:
                             st.info(f"This validation is skipped for **{validator.country}**.")
                        elif unique_sids > 0:
                            ...
                            st.info("No products flagged by this check.")
