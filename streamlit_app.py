...
def check_perfume_price(data, perfumes_df, perfume_category_codes):
    ...
    for index, row in perfume_data.iterrows():
        ...
        for p_index, perfume_row in perfumes_df.iterrows():
            ref_brand = perfume_row['BRAND'].strip().lower()
            ref_product_name = perfume_row['PRODUCT_NAME'].strip().lower()
        ...
        if matched_perfume_row is None: # If no PRODUCT_NAME match, try KEYWORD
             for p_index, perfume_row in perfumes_df.iterrows():
                ref_brand = perfume_row['BRAND'].strip().lower()
                ref_keyword = perfume_row['KEYWORD'].strip().lower()
                ref_product_name = perfume_row['PRODUCT_NAME'].strip().lower() # Still need product name for reference
