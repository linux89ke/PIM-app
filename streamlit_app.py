...
def load_txt_file(filename: str) -> List[str]:
    ...
    Logs an internal warning for missing files but suppresses Streamlit warnings.
    """
    try:
...
def load_flags_mapping() -> Dict[str, Tuple[str, str]]:
    ...
    Includes the 'Jersey flag' (Counterfeit Jersey Check) and 'Restricted Brand' flag.
    """
    try:
        flag_mapping = {
            'Sensitive words': (
            ...
            # --- COUNTERFEIT JERSEY CHECK (Jersey flag) ---
            'Jersey flag': (
...
def load_all_support_files() -> Dict:
    ...
    files = {
        ...
        'restricted_brands': load_txt_file('restricted_brands.txt'), 
...
def check_jersey_flag(data: pd.DataFrame) -> pd.DataFrame:
    """Check for the word 'jersey' in product names (Counterfeit Jersey Check)."""
    ...
    # Flags anything with the word 'jersey'
...
def check_restricted_brand(data: pd.DataFrame, restricted_brands: List[str]) -> pd.DataFrame:
    ...
    if 'BRAND' not in data.columns or not restricted_brands:
        # Check is skipped if restricted_brands list is empty (e.g., file not found)
        return pd.DataFrame(columns=data.columns)
    ...
    if uploaded_file is not None:
        try:
            ...
            if uploaded_file.name.endswith('.csv'):
                # --- Robust CSV reading for common ParserError (semicolon delimiter) ---
                try:
