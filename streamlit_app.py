...
def load_flags_mapping() -> Dict[str, Tuple[str, str]]:
    try:
        ...
        required = ['rejection_reason_code', 'Comment']
        missing = [c for c in required if c not in flags_df.columns]
        if missing:
            logger.error(f"flags.xlsx missing columns: {missing}")
            st.error(f"flags.xlsx missing required columns: {missing}")
            return {}

        flags_df['rejection_reason_code'] = flags_df['rejection_reason_code'].astype(str).str.strip()
        flags_df['Comment'] = (
            flags_df['Comment']
            .astype(str)
            .str.strip()
        ...
        if 'FLAG_NAME' in flags_df.columns:
            ...
            for idx, name in enumerate(flags_df['FLAG_NAME']):
                base = name if name else f"Flag_{flags_df['rejection_reason_code'].iloc[idx].split()[0]}"
                uniq = base
                i = 1
                while uniq in seen:
                    uniq = f"{base}_{i}"
                    i += 1
                seen.add(uniq)
                unique_names.append(uniq)
            flags_df['FLAG_NAME'] = unique_names
        else:
            flags_df['FLAG_NAME'] = "Flag_" + flags_df['rejection_reason_code'].str.split().str[0]

        mapping = {
            row['FLAG_NAME']: (row['rejection_reason_code'], row['Comment'])
            for _, row in flags_df.iterrows()
        }

        logger.info(f"Loaded {len(mapping)} flag → reason mappings")
        st.success(f"Loaded {len(mapping)} flag → reason mappings")
        return mapping

    except FileNotFoundError:
        logger.error("flags.xlsx not found")
        st.error("flags.xlsx not found – required for validation.")
        return {}
    except Exception as e:
...
def load_all_support_files() -> Dict:
    ...
    logger.info("All support files loaded")
    return files
...
def validate_products(data: pd.DataFrame, support_files: Dict, country_validator: CountryValidator) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    ...
    validations = [
        ...
        ("Seller Approved to Sell Perfume", check_seller_approved_for_perfume,
         {'perfume_category_codes': support_files['perfume_category_codes'], 'approved_perfume_sellers': support_files['approved_perfume_sellers'],
...
if not support_files['flags_mapping']:
    st.error("Critical: flags.xlsx could not be loaded. Ensure columns: rejection_reason_code, Comment")
    st.stop()
