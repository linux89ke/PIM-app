...
def check_hidden_brand_in_name(data: pd.DataFrame, brands_list: List[str]) -> pd.DataFrame:
    ...
    PROCESSES IN CHUNKS TO AVOID REGEX OVERFLOW.
    """
    ...
    # 2. Sort brands by length (desc)
    clean_brands = [str(b).strip() for b in brands_list if b and str(b).strip().lower() != 'generic']
    sorted_brands = sorted(clean_brands, key=len, reverse=True)
    ...
    for i in range(0, len(sorted_brands), chunk_size):
        chunk = sorted_brands[i:i+chunk_size]
        # Build pattern: ^(Brand1|Brand2|...)\b
