...
# Visible columns for frontend display only
VISIBLE_COLUMNS = [
...
class CountryValidator:
    COUNTRY_CONFIG = {
        "Kenya": {"code": "KE", "skip_validations": [], "prohibited_products_file": "prohibited_productsKE.txt", "fx_rate": 129.0},
        "Uganda": {"code": "UG", "skip_validations": ["Seller Approve to sell books", "Seller Approved to Sell Perfume", "Counterfeit Sneakers", "Product Warranty"], "prohibited_products_file": "prohibited_productsUG.txt", "fx_rate": 3573.0}
    }
    def __init__(self, country: str):
        ...
        self.fx_rate = self.config["fx_rate"]
...
def propagate_metadata(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df
# --- Validation Functions (keeping all original logic) ---
...
def check_suspected_fake_products(data: pd.DataFrame, suspected_fake_df: pd.DataFrame, fx_rate: float = 129.0) -> pd.DataFrame:
    ...
    try:
        ...
        check_data['price_usd'] = check_data['price_to_use'] / fx_rate
...
def check_generic_brand_issues(data: pd.DataFrame, valid_category_codes_fas: List[str]) -> pd.DataFrame:
    ...
    brand_lower = data['BRAND'].astype(str).str.strip().str.lower()
    return data[data['CATEGORY_CODE'].isin(valid_category_codes_fas) & (brand_lower == 'generic')].drop_duplicates(subset=['PRODUCT_SET_SID'])
...
def validate_products(data: pd.DataFrame, support_files: Dict, country_validator: CountryValidator, data_has_warranty_cols: bool, common_sids: Optional[set] = None):
    ...
    validations = [
        ("Suspected Fake product", check_suspected_fake_products, {'suspected_fake_df': support_files['suspected_fake'], 'fx_rate': country_validator.fx_rate}),
...
st.title("🛍️ Product Validation Tool")
st.markdown("---")
...
if not support_files['flags_mapping']:
    ...
    st.stop()
tab1, tab2, tab3 = st.tabs(["📝 Daily Validation", "📊 Weekly Analysis", "🗄️ Data Lake"])
# -------------------------------------------------
# TAB 1: DAILY VALIDATION WITH ENHANCED UI
# -------------------------------------------------
with tab1:
    ...
    if uploaded_files:
        try:
            ...
            st.success(f"✅ Loaded {len(merged_data):,} rows from {len(uploaded_files)} file(s)")
            ...
            if is_valid:
                ...
                with col1:
                    search_query = st.text_input("🔍 Global Search", placeholder="Search by SID, Name, Brand, or Seller...").strip().lower()
                with col2:
                ...
                c1.metric("Total Products", f"{len(data):,}")
                c2.metric("✅ Approved", f"{len(approved_df):,}", delta=len(st.session_state.manual_approvals) if st.session_state.manual_approvals else None)
                c3.metric("❌ Rejected", f"{len(rejected_df):,}")
                rej_rate = (len(rejected_df)/len(data)*100) if len(data) > 0 else 0
                c4.metric("Rejection Rate", f"{rej_rate:.1f}%")
                ...
                for title, df_flagged in flag_dfs.items():
                    # Filter out manually approved items
                    df_remaining = df_flagged[~df_flagged['PRODUCT_SET_SID'].isin(st.session_state.manual_approvals)].copy()
                   
                    # Apply search filter
                    if search_query:
                        mask = df_remaining.astype(str).apply(lambda x: x.str.contains(search_query, case=False, na=False)).any(axis=1)
                        display_df = df_remaining[mask].copy()
                    else:
                        display_df = df_remaining.copy()
                    with st.expander(f"🏷️ {title} ({len(display_df)} items)", expanded=len(display_df) > 0 and len(display_df) < 50):
                        if not display_df.empty:
                            # Add approval checkbox column
                            display_df.insert(0, "✓ Approve", False)
                           
                            # Select only visible columns
                            cols_to_show = ["✓ Approve"] + [c for c in VISIBLE_COLUMNS if c in display_df.columns]
                           
                            # Configure column display
                            col_config = {
                                "✓ Approve": st.column_config.CheckboxColumn("Approve?", width="small"),
                                "MAIN_IMAGE": st.column_config.ImageColumn("Image", width="small"),
                                "PRODUCT_SET_SID": st.column_config.TextColumn("SID", width="medium"),
                            ...
                            edited_df = st.data_editor(
                                ...
                                key=f"editor_{title}_{len(display_df)}_{search_query}"
                            )
                            ...
                            if newly_approved:
                                ...
                                with col_a:
                                    st.info(f"📋 {len(newly_approved)} item(s) selected for approval")
                                with col_b:
                                    if st.button(f"✅ Confirm Bulk Approval", key=f"btn_{title}"):
                                        st.session_state.manual_approvals.update(newly_approved)
                                        st.success(f"✅ Approved {len(newly_approved)} item(s)")
                                        st.rerun()
                           
                            # Export button
                            st.download_button(
                                f"📥 Export {title} Data",
                                to_excel_flag_data(df_flagged, title),
                                f"{file_prefix}_{title.replace(' ', '_')}_{current_date}.xlsx",
                                key=f"export_{title}"
                            )
                        else:
                            st.success("✅ No issues found in this category")
                # Overall Exports Section
                st.markdown("---")
                ...
                with col1:
                    st.download_button(
                        "📄 Final Report",
                        to_excel(final_report, support_files['reasons']),
                        f"{file_prefix}_Final_Report_{current_date}.xlsx",
                        use_container_width=True
                    )
                with col2:
                    st.download_button(
                        ...
                        f"{file_prefix}_Rejected_{current_date}.xlsx",
                        use_container_width=True
                    )
                with col3:
                    st.download_button(
                        ...
                        f"{file_prefix}_Approved_{current_date}.xlsx",
                        use_container_width=True
                    )
                with col4:
                    st.download_button(
                        "📊 Full Data Export",
                        to_excel_full_data(data, final_report),
                        f"{file_prefix}_Full_Data_{current_date}.xlsx",
                        use_container_width=True
                    )
            else:
...
with tab2:
    ...
    st.info("📤 Upload multiple 'Full Data' files from the Daily tab to view aggregated trends")
    ...
    if weekly_files:
        ...
        if not combined_df.empty:
            ...
            m3.metric("Rejection Rate", f"{(rej_count/total*100):.1f}%")
            m4.metric("Unique Sellers", f"{combined_df['SELLER_NAME'].nunique():,}")
# -------------------------------------------------
# TAB 3: DATA LAKE
# -------------------------------------------------
with tab3:
    st.header("🗄️ Data Lake Audit")
    st.info("📋 View recent validation runs and audit logs")
    try:
        df_audit = pd.read_json('validation_audit.jsonl', lines=True).tail(50)
        st.dataframe(df_audit, use_container_width=True)
    except:
        st.info("ℹ️ No audit log found yet. Run validations to generate logs.")
